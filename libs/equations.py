...
class EuropeanCallSpread:
    def __init__(self):
        ...
        self.y_init = [15,25]               # initial guess for put price
    ...
    def backward_g_tf(self, t, x):
        ...
        return tf.maximum(obj - self.K1, 0)*np.exp(-self.r1*self.T) - \
                2*tf.maximum(obj - self.K2, 0)*np.exp(-self.r1*self.T)
    ...
    def backward_f_tf(self, t, x, y, z):
        ...
        return self.r1*tf.maximum(y - term, 0) - self.r2*tf.minimum(y - term, 0) + \
                term*self.mu
...
class EuropeanPut:
    ...
    def backward_g_tf(self, t, x):
        ...
        return tf.maximum(self.K - x[:,:,-1], 0)*np.exp(-self.r*self.T)
